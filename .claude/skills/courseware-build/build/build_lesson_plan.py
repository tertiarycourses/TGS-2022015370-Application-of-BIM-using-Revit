#!/usr/bin/env python3
"""Generate the WSQ Application of BIM using Revit (TGS-2022015370) Lesson Plan
(LP) DOCX in the Tertiary house format.
...
lunch, tea within, final assessment Day 2 from 3:45pm). Topics/labs come from
...
SCHEDULE = {
 1: ("Introduction to BIM and Revit · BIM Modeling", [
    ("9:30","10:00",30,"admin","Welcome, course introduction, learning outcomes, course outline, ground rules and mandatory digital attendance (AM)"),
    ("10:00","11:00",60,"topic","Topic 1 — Introduction to BIM and Revit: what is BIM, principles, BIM dimensions (3D–8D), value proposition, OIR/AIR/EIR/BEP/PIM/AIM, introduction of Revit for BIM (concepts + demo) — LO1, K1–K4, A1–A2"),
    ("11:00","11:15",15,"break","Tea break"),
    ("11:15","13:00",105,"lab","Hands-on: "+lab_titles([1,2])),
    ("13:00","14:00",60,"lunch","Lunch break (PM digital attendance after lunch)"),
    ("14:00","15:30",90,"topic","Topic 2 — BIM Modeling: parametric elements, walls, curtain walls, floors, ceilings, roofs, stairs, structure, site/topography, massing, generative design, families and materials (concepts + demo) — LO2, K6–K7, A4–A5"),
    ("15:30","15:45",15,"break","Tea break"),
    ("15:45","18:15",150,"lab","Hands-on: "+lab_titles([3,4,5,6])),
    ("18:15","18:30",15,"recap","Day 1 recap and Q&A"),
 ]),
 2: ("BIM Application · BIM Documentation · Final Assessment", [
    ("9:30","9:45",15,"recap","Day 1 recap and mandatory digital attendance (AM)"),
    ("9:45","10:45",60,"topic","Topic 3 — BIM Application: applying and operating BIM, view range/templates, schedules and quantity takeoffs, cost estimates, Revit Systems Analysis (EnergyPlus), interference checks (concepts + demo) — LO3, K5, A6–A8"),
    ("10:45","11:00",15,"break","Tea break"),
    ("11:00","13:00",120,"lab","Hands-on: "+lab_titles([7,8])),
    ("13:00","14:00",60,"lunch","Lunch break (PM digital attendance after lunch)"),
    ("14:00","15:30",90,"lab","Topic 4 — BIM Documentation: BEP/EIR/MIDP/CDE, drawings and specifications, Singapore standards (SVY21, naming, colours), worksharing, BCA e-submission (concepts). Hands-on: "+lab_titles([9,10])+" — LO4, K8–K10, A3, A9"),
    ("15:30","15:45",15,"break","Tea break"),
    ("15:45","16:00",15,"assess","Briefing for Assessment"),
    ("16:00","17:00",60,"assess","Written Assessment (WA) — Short-Answer Questions (SAQ), 1 hour, open book — assesses K1–K10"),
    ("17:00","18:30",90,"assess","Practical Performance (PP) — hands-on Revit BIM tasks on the PP dataset, 1.5 hours, open book — assesses A1–A9. Assessment digital attendance, TRAQOM survey and Assessment Summary Record"),
 ]),
}
...
prodoc.add_version_control(doc,[
 ...
 (C.VERSION.lstrip("v"),C.VERSION_DATE,"Single-source rebuild aligned with the v10 slide deck and Learner Guide — 2-day schedule (9:30am\u20136:30pm) covering 10 hands-on Revit labs across the four topics, with the WA (SAQ, 1 hr) + Practical Performance (1.5 hrs) assessment on Day 2.",C.TRAINER),
])
prodoc.add_toc(doc)
...
info=[("Course Title",C.TITLE),("WSQ Course Reference",C.COURSE_CODE),
      ...
      ("Mode","Instructor-led, hands-on BIM modeling labs in Autodesk Revit"),
      ("Trainer",C.TRAINER)]
...
for a in [C.ASSESSMENT["written"],C.ASSESSMENT["practical"],
          ...
          "Final assessment is conducted on Day 2 from 3:45 pm (briefing, then WA followed by PP).",
          "The WA (SAQ) assesses TSC Knowledge K1–K10; the PP assesses TSC Abilities A1–A9.",C.ASSESSMENT["note"]]:
...
H("Lab Reference (aligned to the TSC Knowledge and Abilities)",1)
tt=doc.add_table(rows=0,cols=3); tt.style="Table Grid"
hdr=tt.add_row().cells
for i,htext in enumerate(["Topic / Learning unit","Alignment","Labs"]):
    set_cell(hdr[i],htext,bold=True,size=10,color=RGBColor(0xFF,0xFF,0xFF),fill=HEADER_FILL)
for tp in C.TOPICS:
