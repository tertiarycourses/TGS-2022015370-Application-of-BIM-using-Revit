...
import os as _os
_key = _os.environ.get("LMS_TMS_API_KEY", "")
if not _key:
    for _p in (_os.path.join(_os.path.dirname(_os.path.dirname(_os.path.abspath(__file__))), "lms_tms_api_key"),
               _os.path.expanduser("~/.claude/lms_tms_api_key")):
        if _os.path.exists(_p):
            _key = open(_p).read().strip(); break
if _key:
    UA["x-api-key"] = _key
...
def doc_text(link):
    ...
    try:
        # plain UA only — never send the LMS service key to Google
